...
The autopilot action runs in `mode: execute`, so it does everything inline — no dispatched follow-up workflows to maintain.
...
4. Open a sprint issue with the `sprint` label to kick off the tracker loop, or leave it empty and the factory cycle will seed ideas on the next heartbeat.
...
| `workflow_run` (CI) completed on `agent/issue-*` | The Test check the tracker loop is gated on just finished. |
