...
export function resetCounter(db: Database): void {
...
async function parseIncrementBody(req: Request): Promise<{ increment: number } | Response> {
...
export async function handleNamedCounterPost(
  ...
  const result = await parseIncrementBody(req);
  if (result instanceof Response) return { response: result };
  const count = incrementNamedCounter(db, name, result.increment);
...
export async function handleCounterPost(
  ...
  const row = db
    ...
    .get(result.increment) as { value: number } | null;
